...
/// <summary>
/// This class implements the Cloud Reco Event handling for the project. 
/// It registers itself at the CloudRecoBehaviour and is notified of new search results as well as error messages
/// The current state is visualized and new results are enabled using the TargetFinder API.
/// The class also draws and handles GUI elements that are used in both phase1 and phase2 GUIs. 
/// </summary>
...
public class CloudRecoEventHandler : MonoBehaviour, ICloudRecoEventHandler
{
    ...
    {	
		...
		if(dataLoaded)
		{
			...
			{ //if any sounds are registered to the targetPicture..
				if(GUI.Button(PlaySoundRect, playButton))
				{
					if(!soundInit)
					{
			...
			if(!(textureTest.Equals(PlayerPrefs.GetString("current_image_name"))))
			{
	...
	//Wrap text to make it fit in the augmented reality textbox
